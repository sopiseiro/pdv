...
public class NumeroDocumentJtabel extends PlainDocument {
  ...
  public NumeroDocumentJtabel(int maxLen, JTable t,float tot)
  {
  ...
  public NumeroDocumentJtabel(int maxLen, int qtdDec)
  {
    this(maxLen, qtdDec, '.', ',');
  }
  ...
  public NumeroDocumentJtabel(int maxLen, int qtdDec, char mil, char dec){
    super();
  ...
  public void insertString(int offs, String str, AttributeSet attrSet) throws BadLocationException {
    ...
    // mostar zeros a esquerda quando o valor for menor que "1"
      ...
      String aux =s.toString().replace(".", "");
      float negativo = Float.parseFloat(aux.replace(",", "."));
      if (negativo > total){
          //super.remove(0, getLength());
          //super.insertString(0, s.toString(), attrSet);
          //tabela.setValueAt("0,00",tabela.getSelectedRow(), 3);
          //System.out.println("testando codicao");
      }
      ...
      float subtotal = 0;
      Object x = "";
      for (int i=0;i<=tabela.getSelectedRow();i++){
          if (tabela.getSelectedRow() == i){
                aux = s.toString();
                aux = aux.replace(".", "");
                subtotal += Float.parseFloat(aux.replace(",", "."));
                //System.out.println("aux: "+aux);
          }else{
                x = tabela.getValueAt(i, 3);
                aux = (String)x;
                aux = aux.replace(".", "");
                subtotal += Float.parseFloat(aux.replace(",", "."));
                //System.out.println("aux: "+aux);
      ...
      float diferencaaplicar = total - subtotal;
      System.out.println("acumulado: "+diferencaaplicar);
      float divisaoparcelas = diferencaaplicar/((tabela.getRowCount())-(tabela.getSelectedRow()+1));
      System.out.println("aplicar demais parcelas: "+tabela.getSelectedRow());
      
      for (int i=tabela.getSelectedRow()+1;i<tabela.getRowCount();i++){
          //System.out.println("indice: "+i);
          if (divisaoparcelas < 0)
              divisaoparcelas = 0;
          tabela.setValueAt(df.format(divisaoparcelas), i, 3);
      }
      subtotal = 0;
      diferencaaplicar = 0;
      divisaoparcelas = 0;
      ...
      //tabela.setValueAt(valor, iPos, iPos);
      //System.out.println(s.toString()+" teste "+subtotal);
