...
public class NumeroDocumentDesconto extends PlainDocument {

  private int iMaxLength;
  private int iQtdDec;
  private String sMil;
  private String sDec;
  public JTextField preini;
  public JTextField prefinal;
  public JComboBox box;
  public JTextField alterar;
  private DecimalFormat df;
  


  // Construtor para indicando apenas o tamanho máximo (incluindo o  ponto decimal), será assumido como padrao 2 decimais
  public NumeroDocumentDesconto(int maxLen,JTextField preini, JTextField prefinal, JComboBox box)
  {
    this(maxLen, 2, '.', ',');
    this.preini = preini;
  ...
  public NumeroDocumentDesconto(int maxLen, int qtdDec)
  {
    this(maxLen, qtdDec, '.', ',');
  }
  ...
  public NumeroDocumentDesconto(int maxLen, int qtdDec, char mil, char dec){
    super();
  ...
  public void insertString(int offs, String str, AttributeSet attrSet) throws BadLocationException {
    ...
    // mostar zeros a esquerda quando o valor for menor que "1"
      ...
      String aux = preini.getText().replace(".", "");
      double valorini = Double.parseDouble(aux.replace(",", "."));
      aux = prefinal.getText().replace(".", "");
      double valorfinal = Double.parseDouble(aux.replace(",", "."));
      aux = s.toString().replace(".", "");
      double alt = Double.parseDouble(aux.replace(",", "."));
      
      if (box.getSelectedItem().equals("Desconto em %")){
          double auxiliar = valorini*alt/100;
          prefinal.setText(df.format(valorini - auxiliar));
      }else
          if (box.getSelectedItem().equals("Acréscimo em %")){
              double auxiliar = valorini*alt/100;
              prefinal.setText(df.format(valorini + auxiliar));
          }else
          if (box.getSelectedItem().equals("Valor Final em R$")){
              prefinal.setText(s.toString());
          }
